...
if [[ -n "${OPENSSH_USER}" ]]; then
    ...
    /usr/sbin/addgroup --gid "${OPENSSH_GROUPID}" "${OPENSSH_GROUP}"
    if [[ -d "${OPENSSH_HOME}" ]]; then
        /usr/sbin/adduser --uid "${OPENSSH_USERID}" --ingroup "${OPENSSH_GROUP}" \
                          --shell "${OPENSSH_SHELL}" --home "${OPENSSH_HOME}" \
                          --gecos '' \
                          --no-create-home \
                          --disabled-password \
                          "${OPENSSH_USER}"
...
# add the newly created user to list of sudoers
echo "${OPENSSH_USER} ALL=(ALL) ALL" >> /etc/sudoers
# cd to the PVC directory (must be mounted at /home/jovyan)
echo "cd /home/jovyan" >> /home/${OPENSSH_USER}/.bashrc
echo "export PATH="/home/${OPENSSH_USER}/.local/bin:\$PATH"" >> /home/${OPENSSH_USER}/.bashrc

# generate host keys if not present
ssh-keygen -A

# Prevent error message: Missing privilege separation directory: /var/run/sshd
